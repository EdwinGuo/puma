...
object Utils {
  ...
  def extractDomainInfo(url: String, topDomainsBC: Broadcast[Set[String]], userDomain: String): DomainInfo = {
    ...
    val subdomainEntropy = if (subdomain != "None") Utils.stringEntropy(subdomain) else 0d
  ...
  /**
    * Calculates string's entropy, interpreted as a distribution on its constituent characters.
    *
  ...
  def topDomain(proxyHost: String, topDomains: Set[String]): Int = {
    ...
    val domain = Utils.extractDomain(proxyHost)
